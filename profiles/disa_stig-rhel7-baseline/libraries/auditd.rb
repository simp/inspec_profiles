...
require 'utils/parser'
  ...
  class AuditDaemonRules < Inspec.resource(1)
    ...
    example "
      ...
      # syntax for auditd >= 2.3
      describe auditd.syscall('open').action do
        it { should eq(['always']) }
      end

      describe auditd.key('sshd_config') do
    ...
    def get_arch(fields)
      fields.each do |field|
        if field.start_with? "arch="
          return field.match(/arch=(\S+)\s?/)[1]
        end
      end
      nil
    end
    ...
    def get_perms(fields)
      fields.each do |field|
        if field.start_with? "perm="
          return field.match(/perm=(\S+)\s?/)[1].scan /\w/
        end
      end
      nil
    end
    ...
    def get_path(fields)
      fields.each do |field|
        if field.start_with? "path="
          return field.match(/path=(\S+)\s?/)[1]
        end
      end
      nil
    end
